...
3. The current release version is fixed at `0.1.12`. This version is explicitly specified when installing or executing the CLI/SDK, unless the user explicitly requests other versions; do not use an unpinned package or runner.
4. If you already have a project, use the package manager corresponding to its lockfile. Do not mix npm, pnpm and Bun to install TQX: use it when there is `bun.lock`
   `bunx @tqx-ai/cli@0.1.12`, use `pnpm dlx @tqx-ai/cli@0.1.12` when there is `pnpm-lock.yaml`; use `npx --yes @tqx-ai/cli@0.1.12` when there is no project package manager context.
...
(cd /tmp && TQX_API_KEY="$TQX_API_KEY_VALUE" \
  npx --yes @tqx-ai/cli@0.1.12 status --json)
