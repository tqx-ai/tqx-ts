...
3. The current release version is fixed at `0.1.8`. This version is explicitly specified when installing or executing the CLI/SDK, unless the user explicitly requests other versions; it is prohibited to use versions without a version
...
4. If you already have a project, use the package manager corresponding to its lockfile. Do not mix npm, pnpm and Bun to install TQX: use it when there is `bun.lock`
   `bunx @tqx-ai/cli@0.1.8`, use `pnpm dlx @tqx-ai/cli@0.1.8` when there is `pnpm-lock.yaml`; use when there is no project package manager context
   `npx --yes @tqx-ai/cli@0.1.8`. Temporary execution takes precedence over global installation unless the user explicitly requests a global installation; in that case, ensure and verify the global pinned package instead.
