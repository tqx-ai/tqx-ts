...
- Do not describe `ACCEPTED`, `PENDING` or `UNKNOWN` as completed; only the order status `FILLED` means fully completed.
...
2. **Check and automatically install the CLI**: First read `references/commands.md`, then run `tqx --help` and `tqx --version`. If the command is unavailable, choose npm, pnpm, or Bun for the current environment and install `@tqx-ai/cli@0.1.7` without mixing package managers. Use a pinned temporary runner by default and keep the same runner prefix and version for the entire task. When the user explicitly requests a global installation, install the pinned package globally, normally with `npm install --global @tqx-ai/cli@0.1.7` or the equivalent selected package-manager command, then verify the global `tqx` binary with `tqx --version` and `tqx --help`; do not satisfy that request with a temporary runner alone.
...
11. **Execute and cross-check**: Run `tqx trading orders --help` before placing an order, then call `orders place` as documented. Save `signal_id`/`order_id`, query with `tqx trading signals get`, `tqx trading orders get`, and `tqx trading orders list`, and cross-check with `tqx trading trades` until the final status is known. `ACCEPTED`, `PENDING`, and `UNKNOWN` are not fills; only `FILLED` means the order is fully filled. On a timeout or unknown result, reuse the original idempotency key and query first; never retry blindly with a new key.
...
3. The current release version is fixed at `0.1.7`. This version is explicitly specified when installing or executing the CLI/SDK, unless the user explicitly requests other versions; it is prohibited to use versions without a version
   `npx @tqx-ai/cli`, `pnpm dlx @tqx-ai/cli` or `bunx @tqx-ai/cli` to avoid silently switching to the new version in the task.
4. If you already have a project, use the package manager corresponding to its lockfile. Do not mix npm, pnpm and Bun to install TQX: use it when there is `bun.lock`
   `bunx @tqx-ai/cli@0.1.7`, use `pnpm dlx @tqx-ai/cli@0.1.7` when there is `pnpm-lock.yaml`; use when there is no project package manager context
   `npx --yes @tqx-ai/cli@0.1.7`. Temporary execution takes precedence over global installation unless the user explicitly requests a global installation; in that case, ensure and verify the global pinned package instead.
5. When the CLI is missing, a fixed version of the temporary runner is automatically selected; the SDK is always installed as a project dependency and does not install globally. If the user already has global `tqx`, use it first
   `command -v tqx` and `tqx --version` are used to verify, use the fixed version runner when the task version is not met, and do not silently upgrade or downgrade global commands.
...
(cd /tmp && TQX_API_KEY="$TQX_API_KEY_VALUE" \
  npx --yes @tqx-ai/cli@0.1.7 status --json)
...
6. If `order_id` is returned, use `orders get` to query the order. Report whether it is submitted, partially filled, filled, canceled, rejected, timed out, or failed based only on the actual order status.
